...
import sifive.fpgashells.ip.xilinx.vcu128mig.{VCU128MIGIOClocksReset, VCU128MIGIODDR, vcu128mig}

case class XilinxVCU128MIGParams(
  address : Seq[AddressSet]
)

class XilinxVCU128MIGPads(depth : BigInt) extends VCU128MIGIODDR(depth) {
  def this(c : XilinxVCU128MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxVCU128MIGIO(depth : BigInt) extends VCU128MIGIODDR(depth) with VCU128MIGIOClocksReset

class XilinxVCU128MIGIsland(c : XilinxVCU128MIGParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x80000000L),"vcu128mig supports upto 2GB depth configuraton")
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxVCU128MIGIO(depth)
    })

    //MIG black box instantiation
    val blackbox = Module(new vcu128mig(depth))
...
class XilinxVCU128MIG(c : XilinxVCU128MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxVCU128MIGIsland(c))
