...
package sifive.fpgashells.ip.xilinx.vcu128mig
...
// IP VLNV: xilinx.com:customize_ip:vcu128mig:1.0
// Black Box

class VCU128MIGIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
  require((depth<=0x80000000L),"VCU128MIGIODDR supports upto 2GB depth configuraton")
...
//reused directly in io bundle for sifive.blocks.devices.xilinxvcu128mig
trait VCU128MIGIOClocksReset extends Bundle {
  //inputs
...
class vcu128mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  require((depth<=0x80000000L),"vcu128mig supports upto 2GB depth configuraton")

  val io = new VCU128MIGIODDR(depth) with VCU128MIGIOClocksReset {
  ...
  ElaborationArtefacts.add(
    "vcu128mig.vivado.tcl",
    """ 
      create_ip -vendor xilinx.com -library ip -version 2.2 -name ddr4 -module_name vcu128mig -dir $ipdir -force
